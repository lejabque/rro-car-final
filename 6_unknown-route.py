...
def line_detect(dir_line):
    ...
    for cnt in contours:
        if (cv.contourArea(cnt) > 100) and (cv.contourArea(cnt) < 1500):
            ...
            mean_val = sum(cv.mean(roi_hsv[:, :, 2], mask))
            # print('Mean V: ', mean_val)
            # noinspection PyBroadException
            # if mean_val < 30:
            if True:
                try:
...
def find_light():
    ...
    # red-orange mask
    lower_red = np.array([130, 135, 140], dtype="uint8")
    ...
    for cnt in contours:
        if (cv.contourArea(cnt) > 70) and (cv.contourArea(cnt) < 300):
            print(cv.contourArea(cnt))
            # x, y, w, h = cv.boundingRect(cnt)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if LINE:
            dir_r = 'F'
            # dir_r = 'F'  # ПОМЕНЯТЬ, ЕСЛИ ЧТО-ТО БУДЕТ НЕ ТАК
            # noinspection PyBroadException
            try:
            ...
            if delta_line == -10000:
                delta_line = last_delta
            print(last_delta - delta_line)
            #if (abs(last_delta - delta_line) > 70) and (abs(last_delta - delta_line) < 150):
            #    delta_line = last_delta
